note
	description: "[
		Concrete storage for INTEGER_64 (int64) data.
		Wraps a standard Eiffel ARRAY [INTEGER_64] for safety and interoperability.
	]"
...
feature {NONE} -- Initialization
	...
	make (n: INTEGER_32)
		...
		require
			n_positive: n >= 0
		do
			create data.make_filled (0, 1, n)
		ensure
			count_set: count = n
		end

	make_from_array (arr: ARRAY [INTEGER_64])
...
feature -- Access
	...
	item_as_int_32 (index: INTEGER_32): INTEGER_32
			-- Read an element as INTEGER_32 (truncated from int64).
		do
			Result := data.item (index).to_integer_32
		end

	item_as_real_64 (index: INTEGER_32): REAL_64
			-- Read an element as REAL_64.
		do
			(create {EXCEPTIONS}).raise ("Type mismatch: storage is INTEGER_64")
		end
